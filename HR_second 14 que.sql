...
SELECT 
    ...
    SUM(e.commission_pct) AS total_commission_payout
FROM 
    employees e
JOIN 
...
If an Employee doesnt earn the commission, than display as No commission. Name the columns appropriately.
SELECT 
    CONCAT(first_name, ' ', last_name) AS employee_name,
    salary,
    CASE 
    ...
    END AS doubled_commission
FROM 
   employees;

--- 8. Display the employees name, department id who have the first name same as another employee in the same department
SELECT 
...
GROUP BY 
    m.employee_id, m.first_name, m.last_name, m.department_id;
...
GROUP BY 
    m.manager_id, e.last_name, e.first_name, e.department_id, e.salary;
