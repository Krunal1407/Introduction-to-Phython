...
SELECT 
    CONCAT(UPPER(SUBSTRING(last_name, 1, 1)), LOWER(SUBSTRING(last_name, 2, 4))) AS modified_last_name
FROM 
    employees;
...
--- 3. Write a query to display the employee's last name and if half of the salary is greater than ten thousand than increase
--- the salary by 10 % else by 11.5% along with the bonus amount of 1500 each. Provide each column an appropriate label.
SELECT 
    last_name AS employee_last_name,
    CASE 
        WHEN salary / 2 > 10000 THEN ROUND(salary * 1.1, 2) + 1500
        ELSE ROUND(salary * 1.115, 2) + 1500
    END AS new_salary_with_bonus
FROM 
    employees;

--- 4. Display the employee ID by Appending two zeros after 2nd digit and 'E' in the end, department id, salary and
the manager name all in Upper cas, if the Manager name consists of 'z' replace it with '$!';
...
--- 5. Write a query that displays the employees last names with the first letter capitalized and all other letters lowercase,
--- and the length of the names, for all employees whose name starts with J1 A1 or M. Give each column an appropriate label.
--- Sort the results by the employees last names;
SELECT 
    (last_name) AS capitalized_last_name,
    LENGTH(last_name) AS name_length
FROM 
    employees
WHERE 
    last_name LIKE 'J1%' OR last_name LIKE 'A1%' OR last_name LIKE 'M%'
ORDER BY 
    last_name;
...
SELECT 
    (first_name) AS capitalized_first_name
FROM 
    employees;
...
--- Name the column as e-mail address. All characters should be in lower cas. Display this along with their First Name.
SELECT 
    first_name,
    CONCAT(LOWER(LEFT(first_name, 1)), LOWER(last_name), '@systechusa.com') AS email_address
FROM 
    employees;
...
FROM 
    employees
WHERE 
    job_id = (SELECT job_id FROM employees WHERE first_name = 'Trenna');
...
WHERE 
    l.city = (SELECT city FROM locations WHERE first_name = 'Trenna');
